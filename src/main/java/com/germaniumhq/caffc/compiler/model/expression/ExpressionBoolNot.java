...
public final class ExpressionBoolNot implements Expression {
    ...
    public static Expression fromAntlr(CompilationUnit unit, AstItem owner, caffcParser.ExBoolNotContext boolNotContext) {
        ...
        if (result.expression instanceof ExpressionInstanceOf) {
            CaffcCompiler.get().warning(result,
                "`not x instanceof Y` should be avoided since it's less readable." +
                    " Use `x not instanceof Y` instead.");
        }
