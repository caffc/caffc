...
public final class TryCatchInstruction implements Statement, ExceptionHandler {
    ...
    public AsmLinearFormResult asLinearForm(AsmBlock block) {
        ...
        // if we reached here, we don't have any exception
        result.instructions.add(new AsmJmp(null,
            this.finallyLabel != null ? this.finallyLabel : this.tryEnd));
        ...
        if (!this.catchBlocks.isEmpty()) {
            ...
            for (CatchBlock catchBlock : this.catchBlocks) {
                AsmLinearFormResult linearCatch = catchBlock.asLinearForm(block);
                result.instructions.addAll(linearCatch.instructions);
            }
        ...
        if (!this.finallyStatements.isEmpty()) {
            ...
            result.instructions.add(this.finallyLabel);

            AsmLinearFormResult linearFinally = this.renderFinallyBlock(block);

            result.instructions.addAll(linearFinally.instructions);
        }
    ...
    private AsmLinearFormResult renderFinallyBlock(AsmBlock block) {
